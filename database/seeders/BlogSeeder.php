        ...
        return [
            ...
            [
                ...
                'content' => [
                    ...
                    'version' => '2.31.0',
                    'blocks' => [
                        ['type' => 'paragraph', 'data' => ['text' => 'Publishing a page does not put it in Google. You have to tell Google your site exists and let it crawl your sitemap. <a href="https://search.google.com/search-console">Google Search Console</a> (GSC) is the free tool for exactly that — open it and sign in with a Google account to begin.']],
                        ['type' => 'header', 'data' => ['text' => 'Set your production APP_URL', 'level' => 2]],
                        ['type' => 'paragraph', 'data' => ['text' => 'Before anything else, set <code>APP_URL</code> in your <code>.env</code> to your real domain (e.g. <code>https://yourdomain.com</code>). Canonical URLs, <code>og:url</code>, the sitemap, and media URLs are all absolute and derived from it — a wrong value quietly breaks indexing.']],
                        ['type' => 'header', 'data' => ['text' => 'Create and verify your property', 'level' => 2]],
                        ['type' => 'paragraph', 'data' => ['text' => 'In <a href="https://search.google.com/search-console">Search Console</a>, click <b>Add property</b>. Google offers two property types:']],
                        ['type' => 'list', 'data' => ['style' => 'unordered', 'items' => [
                            '<b>Domain</b> — covers every subdomain and protocol at once, verified with a DNS TXT record at your registrar. The most thorough option.',
                            '<b>URL prefix</b> — covers one exact address (e.g. <code>https://yourdomain.com</code>). Quickest to set up, and it works with the HTML-tag method this kit supports out of the box.',
                        ]]],
                        ['type' => 'paragraph', 'data' => ['text' => 'For a URL-prefix property, choose the <b>HTML tag</b> verification method. Google shows you a meta tag — copy only the <code>content</code> value (the token):']],
                        ['type' => 'code', 'data' => ['code' => '<meta name="google-site-verification" content="AbC123...your-token..." />']],
                        ['type' => 'paragraph', 'data' => ['text' => 'Paste that token into <b>Admin → Settings → Google site verification</b> and save. The kit renders the tag in every page\'s <code>&lt;head&gt;</code> automatically. Back in GSC, click <b>Verify</b> — it is usually instant.']],
                        ['type' => 'header', 'data' => ['text' => 'Submit your sitemap', 'level' => 2]],
                        ['type' => 'paragraph', 'data' => ['text' => 'The kit serves a sitemap at <code>/sitemap.xml</code> that lists every published post and category, and regenerates it automatically whenever you publish or unpublish. In GSC, open <b>Sitemaps</b>, enter <code>sitemap.xml</code> in the box, and click <b>Submit</b>. The full URL it processes is:']],
                        ['type' => 'code', 'data' => ['code' => 'https://yourdomain.com/sitemap.xml']],
                        ['type' => 'header', 'data' => ['text' => 'Request indexing for key pages', 'level' => 2]],
                        ['type' => 'paragraph', 'data' => ['text' => 'Use the <b>URL Inspection</b> bar at the top of GSC. Paste a full page URL, wait for the live test, then click <i>Request indexing</i>. Do this for your most important pages — the home page and your cornerstone posts — to push them to the front of the crawl queue.']],
                        ['type' => 'header', 'data' => ['text' => 'Check robots.txt is not blocking you', 'level' => 2]],
                        ['type' => 'paragraph', 'data' => ['text' => 'Visit <code>/robots.txt</code>. It should advertise your sitemap and disallow <code>/admin</code> — but never your public content. The kit generates this from your settings, so double-check it after editing them.']],
                        ['type' => 'header', 'data' => ['text' => 'How long until pages appear?', 'level' => 2]],
                        ['type' => 'paragraph', 'data' => ['text' => 'Verification is instant. Sitemap processing takes a day or two. Individual pages are usually indexed within a few days to two weeks. Indexing is never guaranteed — thin, duplicate, or low-value pages may be skipped, so make every page worth indexing.']],
                        ...
                        ['type' => 'list', 'data' => ['style' => 'unordered', 'items' => [
                            '<b>Pages report</b> (formerly Coverage) — which URLs are indexed, and why others were not.',
                            '<b>Performance report</b> — impressions, clicks, average position, and the queries you rank for, once data builds up.',
                            '<b>Sitemaps report</b> — confirms Google is still reading your sitemap and how many URLs it found.',
                        ]]],
                    ],
                ],
                'status' => PostStatus::Published,
